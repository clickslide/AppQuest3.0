...
module.exports = {
  ...
  getStopsByDistance: function(lat, lon, radius, cb){
    ...
    var radiusInDegrees = Math.round(radius/69*100000)/100000;
//    Stop.where('loc').near({ center: [lon, lat], maxDistance: radiusInDegrees }).exec(function(e, results){
//        if(e) throw e;
  ...
  getTimesByStop: function(agency_key, route_id, stop_id, direction_id, cb){
    ...
    function findServices(cb){
      ...
      query[utils.getDayName(today).toLowerCase()] = 1;
        ...
        // @TODO: Update Static Files
      Calendar
        .find(query)
        .where('start_date').lte(/* todayFormatted*/ "20141231" )
        .where('end_date').gte( /* todayFormatted*/ "20141231" )
